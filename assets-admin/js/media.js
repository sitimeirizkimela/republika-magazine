...
$(document).ready(function(){
	...
	$("#showUploadBox").click(function(){
		...
		div = $("<div />").append(
			...
			$("<input name='caption' class='images_"+index+" imgcaption' value='"+txtCaption+"' />")
		).append(
			$("<input name='imgsource' class='source_images_"+index+" imgsource' value='"+txtImgsource+"' />")
		).append(
		...
		onComplete : function(file, responses){
			...
			if(response.length === 4 && response[0] === 'ok')
			{
				...
				$("<div />").attr({
					...
					$("<img src='/files/media/news/images/150x100/"+response[2]+"' alt='"+response[3]+"' title='"+response[3]+"' align=\"left\"  />")
				).append(
...
function insert(obj) {
	if($("#img-"+$(obj).attr('id')).length == 0 && $(".img-box").length < 1) {
		$("<div />").attr({
			...
			$("<img src='"+$(obj).attr('src')+"' alt='"+$(obj).attr('alt')+"' title='"+$(obj).attr('title')+"' width='150' align=\"left\" />")
		).append(
			$("<input type='hidden' name='image[]' value='"+ $(obj).attr('src').split('/').pop() +"' />")
		).append(
